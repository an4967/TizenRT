...
int iotbus_spi_set_config(iotbus_spi_context_h hnd, const struct iotbus_spi_config_s *config)
{
	...
	struct spi_io_config config = { config->bits_per_word, config->frequency, config->chip_select, config->mode };
	int res = ioctl(handle->fd, SPIIOC_SET_CONFIG, (unsigned long)((uintptr_t)&config));
	if (res < 0) {
		ibdbg("set config fail\n");
